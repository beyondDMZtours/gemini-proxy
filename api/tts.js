...
export default async function handler(req, res) {
    ...
    try {
        ...
        // POST 요청: 텍스트를 음성으로 변환 (with timestamps for Lip-Sync)
        if (req.method === 'POST') {
            ...
            console.log(`[TTS] Generating with timestamps for voice: ${voice_id}`);
            ...
            // with-timestamps 응답은 JSON (audio_base64 + alignment)
            const result = await response.json();

            console.log(`[TTS] Success! Characters: ${result.alignment?.characters?.length || 0}`);
